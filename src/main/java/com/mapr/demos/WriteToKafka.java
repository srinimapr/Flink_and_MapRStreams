...
public class WriteToKafka {
  ...
  //Create a set of properties with the producer information.
    Properties properties = new Properties();
    // properties.setProperty("bootstrap.servers", "<kafka-broker>:9092"); // not used by MapR Streams
    //Messages are buffered in the producer for at most the specified time
   ...
   /// create a new DataStream in the application environment
    ...
    //add the FlinkKafkaProducer09 sink to the streams;
  ...
  public static class SimpleStringGenerator implements SourceFunction<String> {
    ...
    public void run(SourceContext<String> ctx) throws Exception {
      while(running) {
    	  //generate the string message to be passed to the producer topic.
    	  //FLINK string is padded with sequence numbers.
        ctx.collect("FLINK-"+ (i++));
        Thread.sleep(10);
      }
    }
